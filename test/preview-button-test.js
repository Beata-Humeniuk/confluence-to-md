...
const { previewPullButton, pullLink, fileUriOf } = require('../src/previewButton');

const md = previewPullButton(new MarkdownIt({ html: true }),
...
assert(html.indexOf('<a class="confluence-to-md-pull"') === 0, 'a bound file starts with the pull button, got: ' + html);
assert(html.includes('href="vscode://beatahumeniuk.confluence-to-md/pull?file=file%3A%2F%2F%2Fw%2FMy%2520page.md"'),
  'the button links back to the extension with the previewed file');
...
assert(!md.render('# Title\n\nBody.\n', { currentDocument: doc }).includes('confluence-to-md-pull'),
  'a file without front matter gets no button');
assert(!md.render('---\ntype: note\n---\n\n# Title\n', { currentDocument: doc }).includes('confluence-to-md-pull'),
  'front matter without a confluence binding gets no button');
assert(!md.render(BOUND, {}).includes('confluence-to-md-pull'), 'no button when the preview does not say which file it shows');
assert(!md.render(BOUND, { currentDocument: { scheme: 'untitled', toString: () => 'untitled:Untitled-1' } }).includes('confluence-to-md-pull'),
  'no button for an unsaved document');

const link = pullLink('vscode-insiders', 'a.b', 'file:///x/a&b.md');
